...
# shingles three-tier cohort.
#
# Runs two parallel base cohort queries (with / without the age filter) then
# intersects each with the same ATLAS VZV and vaccine cohort definitions and
# reports count differences at every tier.
#
# ── Tiers ────────────────────────────────────────────────────────────────────
#   Tier 1  Base cohort   — RD diagnosis + DMARD (±age cutoff)
#   Tier 2  Shingles      — Tier 1 ∩ ATLAS VZV cohort (json_vzv_ids)
#   Tier 3  Vaccinated    — Tier 2 ∩ ATLAS vaccine cohort (json_vaccine_ids)
#
# ── How to run ───────────────────────────────────────────────────────────────
...
# ============================================================================
# ATLAS cohort IDs  (same definitions used in preliminary_tables_shingles.R)
# ============================================================================

message("Fetching ATLAS VZV and vaccine cohort IDs...")
...
message(sprintf("ATLAS VZV cohort:     %d patients", length(json_vzv_ids)))
message(sprintf("ATLAS vaccine cohort: %d patients", length(json_vaccine_ids)))

# ============================================================================
# Shared DMARD / RD concept lists (identical to preliminary_tables_shingles.R)
# ============================================================================
...
shingles_with    <- intersect(json_vzv_ids, ids_with_age)
vaccine_with     <- intersect(json_vaccine_ids, shingles_with)

message(sprintf("WITH age cutoff:  base=%d  shingles=%d  vaccinated=%d",
                length(ids_with_age), length(shingles_with), length(vaccine_with)))
...
shingles_no_age  <- intersect(json_vzv_ids, ids_no_age)
vaccine_no_age   <- intersect(json_vaccine_ids, shingles_no_age)

message(sprintf("WITHOUT age cutoff: base=%d  shingles=%d  vaccinated=%d",
                length(ids_no_age), length(shingles_no_age), length(vaccine_no_age)))
...
new_shingles <- setdiff(shingles_no_age, shingles_with)
new_vaccine  <- setdiff(vaccine_no_age, vaccine_with)

# Age distribution of newly added base-cohort patients
...
message(sprintf("%d additional shingles patients.", length(new_shingles)))
message(sprintf("%d additional vaccinated patients.", length(new_vaccine)))
...
summary_tbl <- tibble::tribble(
  ...
  "Shingles sub-cohort",    length(shingles_with), length(shingles_no_age),
    length(shingles_no_age) - length(shingles_with),
  "Shingles + vaccine",     length(vaccine_with),  length(vaccine_no_age),
    length(vaccine_no_age) - length(vaccine_with)
)

message("\n──────────────────────────────────────────────────────────────────")
message("Shingles cohort size: with vs. without age cutoff")
message("──────────────────────────────────────────────────────────────────")
print(as.data.frame(summary_tbl))
message("──────────────────────────────────────────────────────────────────")

# ============================================================================
# Optional: launch dashboard restricted to newly added patients
# (comment out if you only want the count comparison)
# ============================================================================
#
# launch_trajectory_dashboard(
#   con,
#   person_ids = as.character(new_shingles)
# )
