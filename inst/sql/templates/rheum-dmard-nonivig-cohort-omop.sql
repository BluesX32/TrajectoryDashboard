DROP TABLE IF EXISTS @temp_database_schema.nxw3dm5kCodesets;
CREATE TABLE @temp_database_schema.nxw3dm5kCodesets  
USING DELTA
 AS
SELECT
CAST(NULL AS int) AS codeset_id,
	CAST(NULL AS bigint) AS concept_id  WHERE 1 = 0;
INSERT INTO @temp_database_schema.nxw3dm5kCodesets (codeset_id, concept_id)
...
DROP TABLE IF EXISTS @temp_database_schema.nxw3dm5kqualified_events;
CREATE TABLE @temp_database_schema.nxw3dm5kqualified_events
USING DELTA
AS
SELECT
...
(
 ...
 JOIN @temp_database_schema.nxw3dm5kCodesets cs on (co.condition_concept_id = cs.concept_id and cs.codeset_id = 0)
) C
...
(
 SELECT co.person_id,co.condition_occurrence_id,co.condition_concept_id,co.visit_occurrence_id,co.condition_start_date as start_date, COALESCE(co.condition_end_date, DATEADD(day,1,co.condition_start_date)) as end_date 
 FROM @cdm_database_schema.CONDITION_OCCURRENCE co
 JOIN @temp_database_schema.nxw3dm5kCodesets cs on (co.condition_concept_id = cs.concept_id and cs.codeset_id = 1)
) C
-- End Condition Occurrence Criteria
UNION ALL
-- Begin Condition Occurrence Criteria
...
(
 SELECT co.person_id,co.condition_occurrence_id,co.condition_concept_id,co.visit_occurrence_id,co.condition_start_date as start_date, COALESCE(co.condition_end_date, DATEADD(day,1,co.condition_start_date)) as end_date 
 FROM @cdm_database_schema.CONDITION_OCCURRENCE co
 JOIN @temp_database_schema.nxw3dm5kCodesets cs on (co.condition_concept_id = cs.concept_id and cs.codeset_id = 3)
) C
-- End Condition Occurrence Criteria
UNION ALL
-- Begin Condition Occurrence Criteria
...
(
 SELECT co.person_id,co.condition_occurrence_id,co.condition_concept_id,co.visit_occurrence_id,co.condition_start_date as start_date, COALESCE(co.condition_end_date, DATEADD(day,1,co.condition_start_date)) as end_date 
 FROM @cdm_database_schema.CONDITION_OCCURRENCE co
 JOIN @temp_database_schema.nxw3dm5kCodesets cs on (co.condition_concept_id = cs.concept_id and cs.codeset_id = 4)
) C
-- End Condition Occurrence Criteria
UNION ALL
-- Begin Condition Occurrence Criteria
...
(
 SELECT co.person_id,co.condition_occurrence_id,co.condition_concept_id,co.visit_occurrence_id,co.condition_start_date as start_date, COALESCE(co.condition_end_date, DATEADD(day,1,co.condition_start_date)) as end_date 
 FROM @cdm_database_schema.CONDITION_OCCURRENCE co
 JOIN @temp_database_schema.nxw3dm5kCodesets cs on (co.condition_concept_id = cs.concept_id and cs.codeset_id = 5)
) C
-- End Condition Occurrence Criteria
UNION ALL
-- Begin Condition Occurrence Criteria
...
(
 SELECT co.person_id,co.condition_occurrence_id,co.condition_concept_id,co.visit_occurrence_id,co.condition_start_date as start_date, COALESCE(co.condition_end_date, DATEADD(day,1,co.condition_start_date)) as end_date 
 FROM @cdm_database_schema.CONDITION_OCCURRENCE co
 JOIN @temp_database_schema.nxw3dm5kCodesets cs on (co.condition_concept_id = cs.concept_id and cs.codeset_id = 6)
) C
-- End Condition Occurrence Criteria
UNION ALL
-- Begin Condition Occurrence Criteria
...
(
 SELECT co.person_id,co.condition_occurrence_id,co.condition_concept_id,co.visit_occurrence_id,co.condition_start_date as start_date, COALESCE(co.condition_end_date, DATEADD(day,1,co.condition_start_date)) as end_date 
 FROM @cdm_database_schema.CONDITION_OCCURRENCE co
 JOIN @temp_database_schema.nxw3dm5kCodesets cs on (co.condition_concept_id = cs.concept_id and cs.codeset_id = 7)
) C
-- End Condition Occurrence Criteria
 ) E
...
;
DROP TABLE IF EXISTS @temp_database_schema.nxw3dm5kInclusion_0;
CREATE TABLE @temp_database_schema.nxw3dm5kInclusion_0
USING DELTA
AS
SELECT
...
(
 ...
 FROM @temp_database_schema.nxw3dm5kqualified_events pe
JOIN (
...
(
 ...
 FROM @temp_database_schema.nxw3dm5kqualified_events E
 INNER JOIN
 (
...
;
DROP TABLE IF EXISTS @temp_database_schema.nxw3dm5kInclusion_1;
CREATE TABLE @temp_database_schema.nxw3dm5kInclusion_1
USING DELTA
AS
SELECT
...
FROM @temp_database_schema.nxw3dm5kqualified_events P
JOIN (
...
JOIN @temp_database_schema.nxw3dm5kCodesets cs on (de.drug_concept_id = cs.concept_id and cs.codeset_id = 8)
) C
...
;
DROP TABLE IF EXISTS @temp_database_schema.nxw3dm5kinclusion_events;
CREATE TABLE @temp_database_schema.nxw3dm5kinclusion_events
USING DELTA
AS
SELECT
inclusion_rule_id, person_id, event_id
FROM
(select inclusion_rule_id, person_id, event_id from @temp_database_schema.nxw3dm5kInclusion_0
UNION ALL
select inclusion_rule_id, person_id, event_id from @temp_database_schema.nxw3dm5kInclusion_1) I;
TRUNCATE TABLE @temp_database_schema.nxw3dm5kInclusion_0;
DROP TABLE @temp_database_schema.nxw3dm5kInclusion_0;
TRUNCATE TABLE @temp_database_schema.nxw3dm5kInclusion_1;
DROP TABLE @temp_database_schema.nxw3dm5kInclusion_1;
DROP TABLE IF EXISTS @temp_database_schema.nxw3dm5kincluded_events;
CREATE TABLE @temp_database_schema.nxw3dm5kincluded_events
USING DELTA
AS
SELECT
...
(
 ...
 from @temp_database_schema.nxw3dm5kqualified_events Q
 LEFT JOIN @temp_database_schema.nxw3dm5kinclusion_events I on I.person_id = Q.person_id and I.event_id = Q.event_id
...
;
DROP TABLE IF EXISTS @temp_database_schema.nxw3dm5kcohort_rows;
CREATE TABLE @temp_database_schema.nxw3dm5kcohort_rows
USING DELTA
AS
SELECT
...
( -- first_ends
 ...
 from @temp_database_schema.nxw3dm5kincluded_events I
 join ( -- cohort_ends
-- cohort exit dates
-- By default, cohort exit at the event's op end date
select event_id, person_id, op_end_date as end_date from @temp_database_schema.nxw3dm5kincluded_events
 ) CE on I.event_id = CE.event_id and I.person_id = CE.person_id and CE.end_date >= I.start_date
 ) F
 WHERE F.ordinal = 1
) FE;
DROP TABLE IF EXISTS @temp_database_schema.nxw3dm5kfinal_cohort;
CREATE TABLE @temp_database_schema.nxw3dm5kfinal_cohort
USING DELTA
AS
SELECT
...
(
 ...
 from @temp_database_schema.nxw3dm5kcohort_rows
 ) CR
 ) ST
) GR
...
FROM @temp_database_schema.nxw3dm5kfinal_cohort CO
;
TRUNCATE TABLE @temp_database_schema.nxw3dm5kcohort_rows;
DROP TABLE @temp_database_schema.nxw3dm5kcohort_rows;
TRUNCATE TABLE @temp_database_schema.nxw3dm5kfinal_cohort;
DROP TABLE @temp_database_schema.nxw3dm5kfinal_cohort;
TRUNCATE TABLE @temp_database_schema.nxw3dm5kinclusion_events;
DROP TABLE @temp_database_schema.nxw3dm5kinclusion_events;
TRUNCATE TABLE @temp_database_schema.nxw3dm5kqualified_events;
DROP TABLE @temp_database_schema.nxw3dm5kqualified_events;
TRUNCATE TABLE @temp_database_schema.nxw3dm5kincluded_events;
DROP TABLE @temp_database_schema.nxw3dm5kincluded_events;
TRUNCATE TABLE @temp_database_schema.nxw3dm5kCodesets;
DROP TABLE @temp_database_schema.nxw3dm5kCodesets;