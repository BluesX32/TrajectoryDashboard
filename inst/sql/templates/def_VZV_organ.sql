CREATE TABLE @temp_database_schema.iwjotvptCodesets  
USING DELTA
 AS
SELECT
CAST(NULL AS int) AS codeset_id,
	CAST(NULL AS bigint) AS concept_id  WHERE 1 = 0;
INSERT INTO @temp_database_schema.iwjotvptCodesets (codeset_id, concept_id)
...
( 
 select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4224242,37209444,608800,608996,37209445,37209443,761347,4205455,4171706,4045976,4139215,438961,45770924,376028,440323,36712850,4310159,4224242,45757253,37108968,45581141,45581142,35205738,45561737,42484196,45571453)
) I
) C;
CREATE TABLE @temp_database_schema.iwjotvptqualified_events
USING DELTA
AS
SELECT
...
(
 ...
 JOIN @temp_database_schema.iwjotvptCodesets cs on (co.condition_concept_id = cs.concept_id and cs.codeset_id = 7)
) C
...
;
CREATE TABLE @temp_database_schema.iwjotvptInclusion_0
USING DELTA
AS
SELECT
...
(
 ...
 FROM @temp_database_schema.iwjotvptqualified_events pe
JOIN (
...
(
 ...
 FROM @temp_database_schema.iwjotvptqualified_events E
 INNER JOIN
 (
 -- Begin Correlated Criteria
select 0 as index_id, cc.person_id, cc.event_id
from (SELECT p.person_id, p.event_id 
FROM @temp_database_schema.iwjotvptqualified_events P
JOIN (
 -- Begin Condition Occurrence Criteria
SELECT C.person_id, C.condition_occurrence_id as event_id, C.start_date, C.end_date,
 C.visit_occurrence_id, C.start_date as sort_date
...
(
 SELECT co.person_id,co.condition_occurrence_id,co.condition_concept_id,co.visit_occurrence_id,co.condition_start_date as start_date, COALESCE(co.condition_end_date, date_add(co.condition_start_date, 1)) as end_date 
 FROM @cdm_database_schema.CONDITION_OCCURRENCE co
 JOIN @temp_database_schema.iwjotvptCodesets cs on (co.condition_concept_id = cs.concept_id and cs.codeset_id = 1)
) C
-- End Condition Occurrence Criteria
...
(
 SELECT co.person_id,co.condition_occurrence_id,co.condition_concept_id,co.visit_occurrence_id,co.condition_start_date as start_date, COALESCE(co.condition_end_date, date_add(co.condition_start_date, 1)) as end_date 
 FROM @cdm_database_schema.CONDITION_OCCURRENCE co
 JOIN @temp_database_schema.iwjotvptCodesets cs on (co.condition_concept_id = cs.concept_id and cs.codeset_id = 2)
) C
-- End Condition Occurrence Criteria
) A on A.person_id = P.person_id AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE ) cc 
GROUP BY cc.person_id, cc.event_id
...
(
 SELECT co.person_id,co.condition_occurrence_id,co.condition_concept_id,co.visit_occurrence_id,co.condition_start_date as start_date, COALESCE(co.condition_end_date, date_add(co.condition_start_date, 1)) as end_date 
 FROM @cdm_database_schema.CONDITION_OCCURRENCE co
 JOIN @temp_database_schema.iwjotvptCodesets cs on (co.condition_concept_id = cs.concept_id and cs.codeset_id = 3)
) C
-- End Condition Occurrence Criteria
) A on A.person_id = P.person_id AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE ) cc 
GROUP BY cc.person_id, cc.event_id
...
(
 SELECT co.person_id,co.condition_occurrence_id,co.condition_concept_id,co.visit_occurrence_id,co.condition_start_date as start_date, COALESCE(co.condition_end_date, date_add(co.condition_start_date, 1)) as end_date 
 FROM @cdm_database_schema.CONDITION_OCCURRENCE co
 JOIN @temp_database_schema.iwjotvptCodesets cs on (co.condition_concept_id = cs.concept_id and cs.codeset_id = 4)
) C
-- End Condition Occurrence Criteria
) A on A.person_id = P.person_id AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE ) cc 
GROUP BY cc.person_id, cc.event_id
...
(
 SELECT co.person_id,co.condition_occurrence_id,co.condition_concept_id,co.visit_occurrence_id,co.condition_start_date as start_date, COALESCE(co.condition_end_date, date_add(co.condition_start_date, 1)) as end_date 
 FROM @cdm_database_schema.CONDITION_OCCURRENCE co
 JOIN @temp_database_schema.iwjotvptCodesets cs on (co.condition_concept_id = cs.concept_id and cs.codeset_id = 5)
) C
-- End Condition Occurrence Criteria
) A on A.person_id = P.person_id AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE ) cc 
GROUP BY cc.person_id, cc.event_id
...
(
 SELECT co.person_id,co.condition_occurrence_id,co.condition_concept_id,co.visit_occurrence_id,co.condition_start_date as start_date, COALESCE(co.condition_end_date, date_add(co.condition_start_date, 1)) as end_date 
 FROM @cdm_database_schema.CONDITION_OCCURRENCE co
 JOIN @temp_database_schema.iwjotvptCodesets cs on (co.condition_concept_id = cs.concept_id and cs.codeset_id = 6)
) C
-- End Condition Occurrence Criteria
) A on A.person_id = P.person_id AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE ) cc 
GROUP BY cc.person_id, cc.event_id
...
;
CREATE TABLE @temp_database_schema.iwjotvptinclusion_events
USING DELTA
AS
SELECT
inclusion_rule_id, person_id, event_id
FROM
(select inclusion_rule_id, person_id, event_id from @temp_database_schema.iwjotvptInclusion_0) I;
TRUNCATE TABLE @temp_database_schema.iwjotvptInclusion_0;
DROP TABLE @temp_database_schema.iwjotvptInclusion_0;
CREATE TABLE @temp_database_schema.iwjotvptincluded_events
USING DELTA
AS
SELECT
...
(
 ...
 from @temp_database_schema.iwjotvptqualified_events Q
 LEFT JOIN @temp_database_schema.iwjotvptinclusion_events I on I.person_id = Q.person_id and I.event_id = Q.event_id
...
;
CREATE TABLE @temp_database_schema.iwjotvptcohort_rows
USING DELTA
AS
SELECT
...
( -- first_ends
 ...
 from @temp_database_schema.iwjotvptincluded_events I
 join ( -- cohort_ends
-- cohort exit dates
-- By default, cohort exit at the event's op end date
select event_id, person_id, op_end_date as end_date from @temp_database_schema.iwjotvptincluded_events
 ) CE on I.event_id = CE.event_id and I.person_id = CE.person_id and CE.end_date >= I.start_date
 ) F
 WHERE F.ordinal = 1
) FE;
CREATE TABLE @temp_database_schema.iwjotvptfinal_cohort
USING DELTA
AS
SELECT
...
( --cteEnds
 ...
 FROM @temp_database_schema.iwjotvptcohort_rows c
 JOIN ( -- cteEndDates
 SELECT
 person_id
 ...
 , -1 AS event_type
 FROM @temp_database_schema.iwjotvptcohort_rows
 UNION ALL
 SELECT
 person_id
...
FROM @temp_database_schema.iwjotvptfinal_cohort CO
;
TRUNCATE TABLE @temp_database_schema.iwjotvptcohort_rows;
DROP TABLE @temp_database_schema.iwjotvptcohort_rows;
TRUNCATE TABLE @temp_database_schema.iwjotvptfinal_cohort;
DROP TABLE @temp_database_schema.iwjotvptfinal_cohort;
TRUNCATE TABLE @temp_database_schema.iwjotvptinclusion_events;
DROP TABLE @temp_database_schema.iwjotvptinclusion_events;
TRUNCATE TABLE @temp_database_schema.iwjotvptqualified_events;
DROP TABLE @temp_database_schema.iwjotvptqualified_events;
TRUNCATE TABLE @temp_database_schema.iwjotvptincluded_events;
DROP TABLE @temp_database_schema.iwjotvptincluded_events;
TRUNCATE TABLE @temp_database_schema.iwjotvptCodesets;
DROP TABLE @temp_database_schema.iwjotvptCodesets;