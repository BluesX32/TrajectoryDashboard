...
--   5=SLE2_rs (no desc), 6=DM2_rs (+desc), 7=SSc2_rs (no desc),
--   8=GCA_rs (no desc), 9=RA_rs (no desc), 10=SpA_rs (no desc),
--   11=vasculitis_rs (no desc)
-- ---------------------------------------------------------------------------
rheum_concepts AS (
  SELECT DISTINCT concept_id
  FROM @vocab_schema.concept
  WHERE concept_id IN (
    -- SLE2_rs (5) — 31 unique concept IDs
    ...
    -- DM2_rs (6) base concept IDs — descendants added via UNION below
