...
#   first_rd_date   Earliest RD diagnosis — used for PPX_NOPJP_ONSET filter
...
-- ── First RD encounter ever (used for PPX_NOPJP_ONSET filter) ─────────────
rd_first_ever AS (
...
# ─────────────────────────────────────────────────────────────────────────────
# Who:    Base cohort patients meeting cohort_PJP_infection.json definition
#         with PJP diagnosis ON OR AFTER rd_index_date.
# How:    SQL finds condition_occurrence rows matching SNOMED 438350 (PJP) and
#         descendants.  Result is filtered to index_date >= rd_index_date (only
#         PJP events after RD cohort entry), then intersected with json_pjp_ids
#         to enforce the 3-arm clinical PJP definition.
...
message(sprintf(
  "%d / %d base cohort patients had PJP on or after RD index date (ATLAS-validated 3-arm definition).",
...
#
# PPX filter criteria (using first_rd_date):
#   PJP patients:     ppx_start in [index_date − PPX_PJP_EARLY, index_date − PPX_PJP_LATE]
#                     (T=−42 to T=−14 days before PJP index)
#   Non-PJP patients: ppx_start >= first_rd_date AND ppx_duration > PPX_NOPJP_ONSET
...
table1_pjp <- tbl1_pjp_data |>
  ...
  tab_footnote(
    footnote = paste0(
      ...
      "rheumatic disease diagnosis (first_rd_date) with duration > ", PPX_NOPJP_ONSET,
...
# first_rd_date with duration > PPX_NOPJP_ONSET days are counted.
# ============================================================================
...
# Do NOT filter by json_ppx_ids: that set is PPX-then-PJP patients only.
# Restricting here would place all PPX-but-no-PJP patients in "No prophylaxis"
# and make PPX group PJP IRs artificially ~100%.
...
table3_pjp <- t3_data |>
  ...
  tab_footnote(
    footnote = paste0(
      ...
      "Regimen membership: only prescriptions on or after the patient's earliest ",
      "RD diagnosis with duration > ", PPX_NOPJP_ONSET,
