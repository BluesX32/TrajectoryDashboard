# analysis_dx_overlap_pjp_incident.R
# Disease diagnosis overlap among INCIDENT PJP patients.
#
# ── Outputs ───────────────────────────────────────────────────────────────────
...
#
# ── How to run ────────────────────────────────────────────────────────────────
# Option A: run preliminary_tables_pjp_incident.R first — this script
#   reuses pjp_ids and disease_flags already in the environment.
...
if (STANDALONE || !exists("pjp_ids") || !exists("disease_flags")) {
  message("Sourcing preliminary_tables_pjp_incident.R ...")
...
# ============================================================================
# STEP 1  Filter disease flags to PJP patients and count diagnoses
# ============================================================================

pjp_dx <- disease_flags |>
  filter(person_id %in% pjp_ids) |>
  mutate(across(all_of(DX_COLS), as.integer)) |>
  mutate(n_dx = rowSums(across(all_of(DX_COLS))))

n_total <- nrow(pjp_dx)
n_multi <- sum(pjp_dx$n_dx > 1)

message(sprintf(
  "\n%d / %d (%.1f%%) incident PJP patients have MORE than 1 RD diagnosis.\n",
...
message("── Distribution of RD diagnosis count among incident PJP patients ──")
...
png("output/pjp_incident/upset_dx_overlap_pjp_incident.png",
...
dev.off()
message("Saved: output/pjp_incident/upset_dx_overlap_pjp_incident.png")
...
heatmap_plot <- ggplot(co_long, aes(x = disease_x, y = disease_y, fill = n)) +
  ...
  labs(
    ...
    subtitle = sprintf("Incident PJP patients (N = %d)", n_total),
    x = NULL, y = NULL
  ) +
...
ggsave("output/pjp_incident/heatmap_dx_overlap_pjp_incident.png",
       heatmap_plot, width = 7, height = 6, dpi = 150)
message("Saved: output/pjp_incident/heatmap_dx_overlap_pjp_incident.png")
